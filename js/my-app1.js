...
myApp.onPageBeforeInit('index', function (page) {
    myApp.closePanel();
    var url = "http://interlab.com.ec/wp-json/posts";
    //$('#listNoticias').append('<ul> {{#each this}} <li class="row"> <a href="noticia.html" class="item-content item-link" data-context-name="index.{{@index}}"> <div class="item-inner col-66"> <p class="item-title">{{date}}</p> <div class="item-subtitle">{{title}}</div> </div> <div class="item-media col-33"><img src="{{featured_image.attachment_meta.sizes.thumbnail.url}}" width="150"></div> </a> </li> {{/each}} </ul>');
    $.ajax({
        url: url,
        dataType: "json",
        success: function (data, textStatus, jqXHR) {
...
response.header("Content-Disposition", "attachment; filename=resultados.pdf");
...
function mapa(latitud, longitud, ubicacion, zoom, infor){
    ...
    $("#map").addMarker({
      coords: [latitud, longitud], // Map center (optional)
